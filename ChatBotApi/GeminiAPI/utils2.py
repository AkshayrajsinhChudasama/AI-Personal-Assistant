...
def generalDialog(user_input, chat_history):
    print("[DEBUG] generalDialog called with user_input:", user_input)
    ...
    print("[DEBUG] Extracted data from generalDialog:", extracted_data)
    return extracted_data

def messageGenerator(Task):
    print("[DEBUG] messageGenerator called with Task:", Task)
    ...
    extracted_data = json.loads(eresult.text)
    print("[DEBUG] Extracted data from messageGenerator:", extracted_data)
    return extracted_data

def conflictChecker(conflictResult, dataResult, intent):
    print("[DEBUG] conflictChecker called with conflictResult:", conflictResult)
    print("[DEBUG] dataResult:", dataResult)
   ...
   # first task : you are conflict checker.
    ...
    extracted_data = json.loads(eresult.text)
    print("[DEBUG] Extracted data from conflictChecker:", extracted_data)
    return extracted_data

def conversaction(msg, history):
    print("[DEBUG] conversaction called with msg:", msg)
  ...
  """
    ...
    extracted_data = json.loads(eresult.text)
    print("[DEBUG] Extracted data from conversaction:", extracted_data)
    return extracted_data

def check_task_conflict(new_task, existing_tasks):
    print("[DEBUG] check_task_conflict called with new_task:", new_task)
    print("[DEBUG] Existing tasks:", existing_tasks)
    ...
    if conflicting_tasks:
        print("[DEBUG] Conflicting tasks found:", conflicting_tasks)
        return {"isConflict": True, "conflicting_tasks": conflicting_tasks}
    else:
        print("[DEBUG] No conflicts found")
        return {"isConflict": False, "message": "No conflict"}

def extract_relevant_info(response):
    """
    Extract relevant information from the model response and return it as JSON.
    """

    try:
        if response.candidates:
            candidate = response.candidates[0]
            if candidate.content and candidate.content[0].text:
                text_response = candidate.content[0].text
                try:  
                    start_idx = text_response.find('{')
                    end_idx = text_response.rfind('}') + 1
                    json_data = text_response[start_idx:end_idx]
                    return json.loads(json_data)
                except json.JSONDecodeError as e:
                    return {"error": f"Invalid JSON in response: {e}", "raw_response": text_response} # Include raw response for debugging
            else:
